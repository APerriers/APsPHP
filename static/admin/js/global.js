layui.define(['element', 'form', 'table'], function(exports) {
    ...
    $('#admin-theme-setting').on('click', function() {
        ...
        layer.open({
            ...
            closeBtn: false,
            anim: 2,
            content: $('#aps-theme-tpl').html(),
            success: function(layero, index) {
                $('.aps-themes li').on('click', function() {
                    var theme = $(this).attr('data-theme');
                    $.get(that.attr('href'), {theme : theme}, function(res) {
                        if (res.code == 0) {
                            layer.msg(res.msg);
                        } else {
                            $('body').prop('class', 'aps-theme-'+theme);
                            $('.aps-themes li').removeClass('active');
                            $('#aps-theme-item-'+theme).addClass('active');
                        }
                    }, 'json');
                });
            }
     ...
     * @attr action 搜索请求地址
     */
    $('#apsSearch,#aps-table-search').submit(function() {
        var that = $(this), 
     ...
     * @attr href 过滤请求地址
     */
    $(document).on('click', '.aps-table-a-filter', function(){
